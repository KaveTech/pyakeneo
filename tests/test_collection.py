...
from pyakeneo.resources import *
...
class TestCollectionMock(VCRTestCase):
    ...
    def test_invalid_json_text(self):
        ...
        with self.assertRaises(Exception):
            c = Result.from_json_text(
                requests.Session(), json_text=self.invalide_json_text
            )
    ...
    def test_loading_fetch_next_page(self):
        ...
        self.assertNotEqual(
            twentyonethElement["identifier"], firstElement["identifier"]
        )
        self.assertNotEqual(
            twentyonethElement["identifier"], eleventhElement["identifier"]
        )
        self.assertEqual(len(c.get_page_items()), 10)
